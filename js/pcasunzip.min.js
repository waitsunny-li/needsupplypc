...
// var opt0 ;//初始值
function _init_area(arrycountry, initDisplay){  //初始化函数
  let s = arrycountry

  let opt0 = initDisplay ? initDisplay : ["省份","地级市","市、县级市"]
	for(let i=0;i<s.length-1;i++){
    // document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
    document.getElementById(s[i]).onchange= function(){
      return change(i+1, s, opt0)
    };
	}
	change(0, s, opt0);
}

function change(v, s, opt0){
	var str="0";
	for(let i=0;i<v;i++){
